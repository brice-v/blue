...
void __glfwPollMonitorsNull(void)
{
    ...
    _GLFWmonitor* monitor = ___glfwAllocMonitor("Null SuperNoop 0",
                                              (int) (mode.width * 25.4f / dpi),
                                              (int) (mode.height * 25.4f / dpi));
    ___glfwInputMonitor(monitor, GLFW_CONNECTED, _GLFW_INSERT_FIRST);
}
...
{
    ___glfwFreeGammaArrays(&monitor->null.ramp);
}
...
void ___glfwGetMonitorContentScaleNull(_GLFWmonitor* monitor,
...
void ___glfwGetMonitorWorkareaNull(_GLFWmonitor* monitor,
                                 int* xpos, int* ypos,
...
{
    GLFWvidmode* mode = __glfw_calloc(1, sizeof(GLFWvidmode));
    *mode = getVideoMode();
    *found = 1;
    return mode;
}
...
GLFWbool ___glfwGetGammaRampNull(_GLFWmonitor* monitor, GLFWgammaramp* ramp)
{
    if (!monitor->null.ramp.size)
    {
        ...
        ___glfwAllocGammaArrays(&monitor->null.ramp, 256);
    ...
    ___glfwAllocGammaArrays(ramp, monitor->null.ramp.size);
