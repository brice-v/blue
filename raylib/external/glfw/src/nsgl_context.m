...
static void swapBuffersNSGL(_GLFWwindow* window)
{
    ...
    if (window->ns.occluded)
    {
        ...
        if (interval > 0)
        {
            ...
            const uint64_t frequency = ___glfwPlatformGetTimerFrequency();
            const uint64_t value = ___glfwPlatformGetTimerValue();
...
static GLFWglproc getProcAddressNSGL(const char* procname)
{
    ...
    GLFWglproc symbol = CFBundleGetFunctionPointerForName(__glfw.nsgl.framework,
                                                          symbolName);
...
{
    if (__glfw.nsgl.framework)
        return GLFW_TRUE;

    __glfw.nsgl.framework =
        CFBundleGetBundleWithIdentifier(CFSTR("com.apple.opengl"));
    if (__glfw.nsgl.framework == NULL)
    {
...
GLFWAPI id _glfwGetNSGLContext(GLFWwindow* handle)
{
