...
static char* getMonitorName(CGDirectDisplayID displayID, NSScreen* screen)
{
    ...
    if (screen)
    {
        if ([screen respondsToSelector:@selector(localizedName)])
        {
            ...
            if (name)
                return ___glfw_strdup([name UTF8String]);
        }
    }
    ...
    char* name = __glfw_calloc(size + 1, 1);
...
void __glfwPollMonitorsCocoa(void)
{
    ...
    CGDirectDisplayID* displays = __glfw_calloc(displayCount, sizeof(CGDirectDisplayID));
    CGGetOnlineDisplayList(displayCount, displays, &displayCount);

    for (int i = 0;  i < __glfw.monitorCount;  i++)
        __glfw.monitors[i]->ns.screen = nil;

    _GLFWmonitor** disconnected = NULL;
    uint32_t disconnectedCount = __glfw.monitorCount;
    ...
    for (uint32_t i = 0;  i < displayCount;  i++)
    {
        ...
        _GLFWmonitor* monitor = ___glfwAllocMonitor(name, size.width, size.height);
        ...
        ___glfwInputMonitor(monitor, GLFW_CONNECTED, _GLFW_INSERT_LAST);
    }
...
{
    GLFWvidmode current;
    ___glfwGetVideoModeCocoa(monitor, &current);
...
void ___glfwGetMonitorContentScaleCocoa(_GLFWmonitor* monitor,
...
void ___glfwGetMonitorWorkareaCocoa(_GLFWmonitor* monitor,
                                  int* xpos, int* ypos,
                                  int* width, int* height)
{
    ...
    if (ypos)
        *ypos = __glfwTransformYCocoa(frameRect.origin.y + frameRect.size.height - 1);
    if (width)
...
GLFWvidmode* ____glfwGetVideoModesCocoa(_GLFWmonitor* monitor, int* count)
{
    ...
    *count = 0;
    ...
    GLFWvidmode* result = __glfw_calloc(found, sizeof(GLFWvidmode));
...
GLFWbool ___glfwGetGammaRampCocoa(_GLFWmonitor* monitor, GLFWgammaramp* ramp)
{
    ...
    CGGammaValue* values = __glfw_calloc(size * 3, sizeof(CGGammaValue));
...
void ____glfwSetGammaRampCocoa(_GLFWmonitor* monitor, const GLFWgammaramp* ramp)
{
    ...
    CGGammaValue* values = __glfw_calloc(ramp->size * 3, sizeof(CGGammaValue));
...
GLFWAPI CGDirectDisplayID _glfwGetCocoaMonitor(GLFWmonitor* handle)
{
