...
static void closeJoystick(_GLFWjoystick* js)
{
    ...
    for (int i = 0;  i < CFArrayGetCount(js->ns.axes);  i++)
        __glfw_free((void*) CFArrayGetValueAtIndex(js->ns.axes, i));
    CFRelease(js->ns.axes);

    for (int i = 0;  i < CFArrayGetCount(js->ns.buttons);  i++)
        __glfw_free((void*) CFArrayGetValueAtIndex(js->ns.buttons, i));
    CFRelease(js->ns.buttons);

    for (int i = 0;  i < CFArrayGetCount(js->ns.hats);  i++)
        __glfw_free((void*) CFArrayGetValueAtIndex(js->ns.hats, i));
                          ...
                          IOHIDDeviceRef device)
{
    ...
    {
        if (__glfw.joysticks[jid].ns.device == device)
            return;
    }
    ...
    for (CFIndex i = 0;  i < CFArrayGetCount(elements);  i++)
    {
        ...
        if (target)
        {
            _GLFWjoyelementNS* element = __glfw_calloc(1, sizeof(_GLFWjoyelementNS));
                           ...
                           IOHIDDeviceRef device)
{
    ...
    {
        if (__glfw.joysticks[jid].connected && __glfw.joysticks[jid].ns.device == device)
        {
...
GLFWbool ___glfwInitJoysticksCocoa(void)
{
    ...
    __glfw.ns.hidManager = IOHIDManagerCreate(kCFAllocatorDefault,
    ...
    if (!matching)
    {
        ___glfwInputError(GLFW_PLATFORM_ERROR, "Cocoa: Failed to create array");
        return GLFW_FALSE;
    }
    ...
    IOHIDManagerSetDeviceMatchingMultiple(__glfw.ns.hidManager, matching);
    CFRelease(matching);

    IOHIDManagerRegisterDeviceMatchingCallback(__glfw.ns.hidManager,
                                               &matchCallback, NULL);
    IOHIDManagerRegisterDeviceRemovalCallback(__glfw.ns.hidManager,
                                              &removeCallback, NULL);
    IOHIDManagerScheduleWithRunLoop(__glfw.ns.hidManager,
                                    CFRunLoopGetMain(),
                                    kCFRunLoopDefaultMode);
    IOHIDManagerOpen(__glfw.ns.hidManager, kIOHIDOptionsTypeNone);
...
void ___glfwTerminateJoysticksCocoa(void)
{
    for (int jid = 0;  jid <= GLFW_JOYSTICK_LAST;  jid++)
    {
    ...
    if (__glfw.ns.hidManager)
    {
        CFRelease(__glfw.ns.hidManager);
        __glfw.ns.hidManager = NULL;
    }
}


GLFWbool __glfwPollJoystickCocoa(_GLFWjoystick* js, int mode)
{
    if (mode & _GLFW_POLL_AXES)
    {
        for (CFIndex i = 0;  i < CFArrayGetCount(js->ns.axes);  i++)
        {
            ...
            if (size == 0)
                ____glfwInputJoystickAxis(js, (int) i, 0.f);
            else
            {
                const float value = (2.f * (raw - axis->minimum) / size) - 1.f;
                ____glfwInputJoystickAxis(js, (int) i, value);
            }
        }
    }
    ...
    if (mode & _GLFW_POLL_BUTTONS)
    {
        for (CFIndex i = 0;  i < CFArrayGetCount(js->ns.buttons);  i++)
        {
            ...
            ____glfwInputJoystickButton(js, (int) i, state);
        }
        ...
        for (CFIndex i = 0;  i < CFArrayGetCount(js->ns.hats);  i++)
        {
            ...
            ____glfwInputJoystickHat(js, (int) i, states[state]);
        }
    }
...
const char* __glfwGetMappingNameCocoa(void)
{
    return "Mac OS X";
}

void __glfwUpdateGamepadGUIDCocoa(char* guid)
{
