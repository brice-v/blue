                             ...
                             int32_t refresh)
{
    ...
    monitor->modeCount++;
    monitor->modes =
        __glfw_realloc(monitor->modes, monitor->modeCount * sizeof(GLFWvidmode));
...
void __glfwAddOutputWayland(uint32_t name, uint32_t version)
{
...
#ifdef WL_OUTPUT_NAME_SINCE_VERSION
    version = ___glfw_min(version, WL_OUTPUT_NAME_SINCE_VERSION);
#else
    version = 2;
#endif

    struct wl_output* output = wl_registry_bind(__glfw.wl.registry,
                                                name,
                                                &wl_output_interface,
                                                version);
    if (!output)
    ...
    _GLFWmonitor* monitor = ___glfwAllocMonitor("", 0, 0);
    monitor->wl.scale = 1;
...
void ___glfwGetMonitorContentScaleWayland(_GLFWmonitor* monitor,
...
void ___glfwGetMonitorWorkareaWayland(_GLFWmonitor* monitor,
                                    int* xpos, int* ypos,
...
GLFWvidmode* ____glfwGetVideoModesWayland(_GLFWmonitor* monitor, int* found)
{
